...
- Commit/revision reviewed: mentor-follow-up working tree over `44af601`
- Date: 2026-09-10
...
- Mentor-follow-up fixes passed a new full `MYSQL_PORT=13306 npm run verify` gate:
  Harness checks, unit 221/221, integration 77/77, E2E 23/23, and build. A separate
  read-only reviewer reran focused unit 12/12 and MySQL integration 9/9, found no
  lock inversion or functional defect, and approved the batch.
...
| MED-06  | Medium   | Mentor PR #9 comment on `BookingsService.create`                                 | The transaction mixed idempotency, lock acquisition, persistence, response mapping, and completion, obscuring the lock order during later changes. | Extract ordered transaction steps; preserve `idempotency -> room -> room-time` and prove concurrent distinct-key creates wait at the shared room lock.               | Codex | Fixed       | `createInTransaction`; focused MySQL concurrent-lock test; full gate; independent re-review found no lock inversion.                                                                          |
| LOW-01  | Low      | Mentor PR #9 comment on the `RoomType` response lookup                           | The lookup selected description and audit columns that the create response never exposes, repeating earlier focused-projection feedback.           | Select only `id` and `name`; keep a query-shape integration assertion and record the reusable lesson.                                                                | Codex | Fixed       | Focused MySQL projection assertion rejects description and audit columns; independent re-review confirmed the query shape.                                                                    |
| LOW-02  | Low      | Mentor PR #9 comment on idempotency completion                                   | Hydrated-row `save` obscured that completion changes only status and stored response fields.                                                       | Use a direct, typed `manager.update` after the already-held idempotency lock.                                                                                        | Codex | Fixed       | Create/replay integration verifies the completed row and original response; independent re-review confirmed transaction/lock scope.                                                           |
...
- Mentor follow-up fixes have passed final independent re-review. P4-T07 remains
  responsible for the final phase-wide review after later slices.
