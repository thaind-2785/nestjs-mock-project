...
| R33-05 | Medium   | five surviving mutations, missing `P5-T05` checks             | Lease renewal, the lease precondition, the claim guard, the delivery status guard, and the SMTP timeouts were all unguarded; "crash-after-accept", "concurrent worker" and the Gmail adapter contract had no test                                                                                                                                                                                                                                                                    | Fixed, then corrected. The first fix left two of the five mutations alive and the reviewer said so after re-checking the committed code. `locked_by` is now pinned by the stale-recipient test; the delivery `status = 'PENDING'` guard is accepted as unreachable defence-in-depth, because with the outbox written first, reaching that statement means the claim was held and nothing else can have resolved the delivery meanwhile | Four of the five mutations fail; the fifth is documented as unreachable rather than claimed as pinned                        |
...
| R33-07 | Low      | lock order between the two transactions                       | `claimWork` locked outbox then delivery; the result path locked delivery then outbox, which MySQL resolved as `ER_LOCK_DEADLOCK`. Recovered through the lease, so nothing corrupted, but it cost a lease period                                                                                                                                                                                                                                                                      | Fixed in a follow-up. The first attempt never applied - the edit's anchor no longer matched after an earlier lint fix, and the script did not assert it - so `failPermanently` still locked `email_deliveries` before the outbox and the reviewer reproduced the deadlock on the committed code. The read is deleted rather than reordered: the repository addresses the delivery by event when no id is given                         | Integration test covers the permanent failure that meets an existing delivery row, which is the path the deleted read served |
...
- **Two claims in the first fix commit were wrong.** It said five mutations now fail;
  two did not. It said every path locks outbox before delivery, while
  `failPermanently` still did the opposite. The reviewer re-verified the committed
  code and caught both. The record above is corrected rather than left standing, and
  the follow-up commit says so.
