...
describe('Phase 4 booking foundation persistence', () => {
  ...
  it('reverts only the Phase 4 schema and reapplies it cleanly', async () => {
    // Five later migrations are stacked on this one and come off first: the Phase 7
    // run ledger, the Phase 6 export schema, the backlog index, the acceptance record,
    // then the delivery schema. The two delivery schemas each refuse their own revert
    // once the evidence they protect exists, which this suite never writes; the index,
    // the export schema and the ledger guard nothing here and always revert. The count
    // is deliberately explicit - a new migration should make a maintainer look at this
